...
class TestExperimentalAPIAreAvailable(TestCase):
    ...
    def test_list_physical_devices(self):
        ...
        parameters = inspect.signature(tf.config.experimental.list_physical_devices).parameters
    ...
    def test_virtual_device_configuration(self):
        ...
        parameters = inspect.signature(tf.config.experimental.VirtualDeviceConfiguration).parameters
    ...
    def test_set_visible_devices(self):
        ...
        parameters = inspect.signature(tf.config.experimental.set_visible_devices).parameters
        self.assertTrue('devices' in parameters)
        self.assertTrue('device_type' in parameters)
        self.assertEqual(2, len(parameters))

    def test_set_virtual_device_configuration(self):
        self.assertTrue("set_virtual_device_configuration" in dir(tf.config.experimental))
        parameters = inspect.signature(tf.config.experimental.set_virtual_device_configuration).parameters
